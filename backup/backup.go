...
import (
	...
	"github.com/AzureOSK/sakamichi-token-extractor/keybag"
	"github.com/AzureOSK/sakamichi-token-extractor/kvarchive"
...
// OpenPath opens a MobileBackup at an explicit filesystem path.
func OpenPath(dir string) (*MobileBackup, error) {
	var backup MobileBackup
	absDir, err := filepath.Abs(dir)
	if err != nil {
		return nil, err
	}
	backup.Dir = filepath.Clean(absDir)

	tmp := filepath.Join(backup.Dir, "Manifest.plist")
	r, err := os.Open(tmp)
	if err != nil {
		return nil, err
	}
	defer r.Close()
	if err := plist.Unmarshal(r, &backup.Manifest); err != nil {
		return nil, err
	}
...
// Open opens a MobileBackup directory corresponding to a backup identifier in
// the platform's default MobileSync location.
func Open(guid string) (*MobileBackup, error) {
	var home string
	var dir string
	if runtime.GOOS == "windows" {
		home = os.Getenv("APPDATA")
		dir = filepath.Join(home, "Apple Computer", "MobileSync", "Backup", guid)
	} else {
		home = os.Getenv("HOME")
		dir = filepath.Join(home, "Library", "Application Support", "MobileSync", "Backup", guid)
	}
	return OpenPath(dir)
}
